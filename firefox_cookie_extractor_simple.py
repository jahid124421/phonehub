...
def find_firefox_profile():
    ...
    if not profile_base.exists():
        print(f"[X] Firefox profile directory not found: {profile_base}")
        return None
    ...
    if not profiles:
        print(f"[X] No Firefox profiles found in {profile_base}")
        return None
    
    # Use the first profile found (usually default)
    profile = profiles[0]
    print(f"[OK] Found Firefox profile: {profile.name}")
    return profile
...
def extract_cookies_from_firefox(profile_dir):
    ...
    if not cookies_db.exists():
        print(f"[X] Firefox cookies database not found: {cookies_db}")
        return {}
    ...
    try:
        ...
        print(f"[X] Error copying Firefox database: {e}")
    ...
    try:
        ...
        print(f"[X] Error reading Firefox cookies: {e}")
        return {}
    finally:
        # Clean up temp file
        if temp_db.exists():
...
def login_to_omniroute():
    ...
    try:
        ...
        if response.status_code != 200:
            print(f"[X] OmniRoute login failed: {response.status_code}")
            return None
        ...
        if not auth_token:
            print("[X] No auth token received from OmniRoute")
            return None
        
        print("[OK] Logged into OmniRoute")
        return auth_token
        
    except requests.exceptions.ConnectionError:
        print("[X] Cannot connect to OmniRoute - make sure it's running!")
...
def get_existing_providers(auth_token):
    ...
    try:
        ...
        print(f"[!] Could not fetch existing providers: {e}")
        return set()
...
def main():
    ...
    print("Step 1: Locating Firefox profile...")
    profile = find_firefox_profile()
    
    if not profile:
        print("\n[X] Could not find Firefox profile")
    ...
    print("Step 3: Connecting to OmniRoute...")
    ...
    print("Step 4: Checking existing providers...")
    ...
    print("Step 5: Adding providers to OmniRoute...\n")
    ...
    print(f"[OK] Successfully Added: {len(results['added'])}")
